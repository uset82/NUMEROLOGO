...
// CÁLCULO DEL PINÁCULO PARA NOEMI MEZA - 25/10/1948
// Usando el sistema completo implementado con reglas de verificación
...
class PinaculoCalculator {
  ...
  static calculateNameNumber(name) {
    ...
    for (let char of name.replace(/\s+/g, '')) {
      ...
      if (this.caldeo[upperChar]) {
        const value = this.caldeo[upperChar];
        sum += value;
        breakdown.push(`${upperChar}=${value}`);
      }
    }
    
    console.log(`\n  Desglose Caldeo: ${breakdown.join(' + ')}`);
  ...
  static calculateComplete(name, day, month, year) {
    ...
    console.log(`\nCálculo de D:`);
    console.log(`  D2 (reducidos): ${results.A} + ${results.B} + ${results.C} = ${D2} → ${D_base}`);
    console.log(`  D1 (sin reducir): ${month} + ${day} + ${year} = ${D1}`);
    ...
    console.log(`\nCálculo de H:`);
    console.log(`  H2 (reducidos): ${results.A} + ${results.C} = ${H2} → ${H_base}`);
    console.log(`  H1 (sin reducir): ${month} + ${year} = ${H1}`);
    ...
    console.log(`W (TRIPLICIDAD) = ${name} (sistema Caldeo)`);
...
// CALCULAR PARA NOEMI MEZA - 25/10/1948
const resultado = PinaculoCalculator.calculateComplete('NOEMI MEZA', 25, 10, 1948);

// Guardar resultado
const fs = require('fs');
fs.writeFileSync('pinaculo-noemi-meza.json', JSON.stringify({
  nombre: "Noemi Meza",
  fecha: "25/10/1948",
  pinaculo: resultado,
...
console.log('\n💾 Resultado guardado en: pinaculo-noemi-meza.json');